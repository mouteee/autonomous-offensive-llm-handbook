...
The turn-ratio measurement in chapter 01 reads session transcripts from a directory on disk and maps each one to a scan. The directory also contains sessions unrelated to target scans, including documentation and review work. Those sessions can change the transcript count without changing any scan. A new count may therefore differ from `[[stats:llm_ratio.coverage.cc_transcripts_seen]]` even when target activity has not changed.

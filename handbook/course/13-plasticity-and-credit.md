...
2. **Update weights on outcomes.** `[[code:controller/plasticity.py:LearnedWeights]]` applies the three-factor rule to every traced pair and clips symmetrically:
...
3. **Combine the readouts.** Under `learn=True` the family score's `w_dot` term is the fixed noise readout plus the learned table's mean over active units, on the same normalization, so learning shows up as a shift on the exact scale lesson 12 established. Frozen, `[[code:controller/plasticity.py:PlasticMbController]]` is the static controller with different bookkeeping: habituation still updates, because that is episode dynamics rather than learning, while no trace is laid and every weight stays at zero.
...
Open `/tmp/plasticity-trace.json` and read the ledger phase by phase. In the reward phase, the first credited outcome moves total weight by [[stats:course.plasticity.first_reward_delta]] and the winning family's learned readout to [[stats:course.plasticity.first_reward_w_dot]]. That is `eta * modulation` spread over the active units; the trace exposes the arithmetic. In the delayed phase, two decisions are made back to back and their outcomes arrive in reverse order; both land, each on its own decision. In the stale phase, a family's reward arrives after four further selects have decayed its traces: its readout moves from [[stats:course.plasticity.stale_fam_a_w_dot_before]] to only [[stats:course.plasticity.stale_fam_a_w_dot_after]], while the other family's fresher traces capture credit up to [[stats:course.plasticity.stale_fam_b_w_dot_after]]. That leak is credit interference between concurrent learning signals, and it is a mechanism property, not a bug in the demo; the originating project's research record measured the same effect, and [the evidence register](../appendix-f-evidence-register.md) records what that came to. In the saturation phase, oversized rewards pin [[stats:course.plasticity.first_clipped_count]] keys at the bound on the first clipped update, and the final update's total change is [[stats:course.plasticity.saturated_delta]]: a saturated weight has stopped carrying information, which is the honest cost of clipping.

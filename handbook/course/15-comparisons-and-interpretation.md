...
The committed run covers [[stats:course.research.controllers]] controller configurations over [[stats:course.research.worlds]] worlds at [[stats:course.research.seeds]] seed, giving [[stats:course.research.conditions]] conditions. That is deliberately tiny. It demonstrates the protocol, not statistical power, and no significance arithmetic is computed over it on purpose: a handful of seeds answers "does the pipeline reproduce" and cannot answer "which policy is better", and pretending otherwise is the exact failure the evidence register exists to catch.

## Implement it

1. **Freeze first.** `[[code:controller/research.py:freeze]]` canonicalizes the design and stamps `[[code:controller/research.py:manifest_digest]]` over exactly that body. Everything downstream carries the digest, which is what makes "the runs match the design" checkable instead of asserted.

2. **Refuse drift.** `[[code:controller/research.py:verify_manifest]]` recomputes the digest before running or analyzing; a manifest edited after freezing (one seed swapped, one world added) is refused with instructions to re-freeze. Changing the design is always allowed; changing it silently is what is not.

3. **Honor the worlds' delays.** `[[code:controller/research.py:run_condition]]` delivers each outcome `world.delay` steps after its decision and flushes what remains when the episode ends. The delayed-credit and decoy-delay worlds exist for this: a policy whose credit lands on "whatever ran when the reward arrived" falls apart under delay, and the decision-keyed contract from lesson 10 is what keeps attribution attached to the decision that earned it.

4. **Save raw before summarizing.** `[[code:controller/research.py:run_all]]` writes a raw file per condition (the full trace, the total, the condition and the manifest digest) before any summary exists. `[[code:controller/research.py:analyze]]` then reads only those files: per condition, the total reward, the regret against the world's best fixed family in hindsight, the repeat ratio (the fraction of steps that chose the same family as the step before), and the first step whose delivered reward cleared the manifest's payout threshold. A condition the manifest names but the raw directory lacks appears as a missing row rather than vanishing, and a raw file produced under a different manifest is refused by name.
...
Open `data/course/research-summary.json` and practice on a single row. The learning bandit on the decoy world reads: total [[stats:course.research.linucb_decoy_total]], regret [[stats:course.research.linucb_decoy_regret]] against the best fixed family, repeat ratio [[stats:course.research.linucb_decoy_repeat_ratio]], first payout at step [[stats:course.research.linucb_decoy_first_payout]]. The scoped reading, in full: on one seed of one synthetic decoy world, under this feedback definition, the learning bandit escaped the decoy and finished near the hindsight baseline. Every clause earns its place; drop "one seed" and the sentence claims replication it does not have; drop "synthetic" and it claims relevance to security testing it does not have; drop "under this feedback definition" and it forgets that reward is a design choice.

The frozen twin is the same arithmetic with learning off: total [[stats:course.research.linucb_frozen_decoy_total]] on the same world, identical to the static baselines, because a frozen bandit with no learned state degenerates to a constant family choice. That pair (learning on against learning off, same policy, same world, same seed) is the cleanest comparison in the table, and it is a comparison of configurations, not of algorithms. Meanwhile the static priority baseline holds regret [[stats:course.research.priority_delayed_regret]] on the delayed-credit world, where the host's static rank happens to be right, and rides the decoy to [[stats:course.research.priority_decoy_total]] on the world where it is wrong: the same policy is the best and the worst row depending on the world, which is why no row generalizes past its world.
...
In the broken world the static priority baseline does what a static rank must: it pulls the highest-rated family every step, and every pull is a `tool_error`: [[stats:course.compare.error-pit-broken.priority.statuses.tool_error]] of them, for a total of [[stats:course.compare.error-pit-broken.priority.total_reward]]. The bandit touches the broken tool [[stats:course.compare.error-pit-broken.linucb.statuses.tool_error]] time, learns from the error feedback, and finishes at [[stats:course.compare.error-pit-broken.linucb.total_reward]], so its lead over the baseline reads [[stats:course.compare.error-pit-broken.linucb_minus_priority]]. Repair the tool and rerun: the same two policies, the same seed, and the lead is [[stats:course.compare.error-pit-repaired.linucb_minus_priority]]: the baseline is optimal again and the bandit pays its ordinary exploration tax. Now write the sentence the exercise exists for: almost the entire broken-world difference came from tool health, not controller quality. A comparison that does not report tool health beside its totals is reporting the pit, not the policies. Note also what the hindsight bar did: the broken world's best fixed family is not the broken one, because hindsight is computed over effective rewards: an unreachable table value cannot set the regret bar.

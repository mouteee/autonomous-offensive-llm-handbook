...
Detection confidence is folded in rather than ignored, [`tool_recommender.py:_field_confidence`](../core/tool_recommender.py). A boost earned from something the fingerprinter is only half sure about gets attenuated by that confidence before it multiplies, so weak evidence nudges the score and strong evidence moves it. The effect shows up on ambiguous targets, and most targets are ambiguous.

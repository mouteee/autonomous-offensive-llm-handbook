...
[^num-1]: 200 is the HTTP status code for OK, the protocol constant this entry is about: the whole defect is a success status believed over the body beneath it. It identifies a response rather than counting anything.

[^num-2]: 200 is the HTTP status code for OK, the protocol constant this entry is about: the whole defect is a success status believed over the body beneath it. It identifies a response rather than counting anything.

[^num-3]: 200 is the HTTP status code for OK, the protocol constant this entry is about: the whole defect is a success status believed over the body beneath it. It identifies a response rather than counting anything.

[^num-4]: one field is a spelled quantity counting a code artifact: how far the structured proof sat from the field the grader actually read, in this genericized incident rather than in a shape core/ carries

...
1. **Write the solver.** [`controller/linucb.py:solve`](../../core/controller/linucb.py) is Gaussian elimination with partial pivoting, and [`controller/linucb.py:identity`](../../core/controller/linucb.py) builds the starting matrix. The matrices are the identity plus a sum of outer products, so they are always solvable; the pivot exists so a corrupted snapshot produces a loud error instead of a quiet wrong answer.

2. **Score a family.** [`controller/linucb.py:family_score`](../../core/controller/linucb.py) implements the two equations this lesson is about:
...
5. **Freeze by default.** [`controller/__init__.py:make_controller`](../../core/controller/__init__.py) builds the bandit frozen; frozen mode performs identical selection arithmetic and leaves `A` and `b` untouched, answering `learning is frozen` to every outcome. Research code opts in explicitly.
...
This is a toy calculation, not a benchmark. With `alpha = 0.5`, the committed trace shows exactly these values: the first score is 0.5, and after one unit of reward the same family at the same `x` scores 0.853553, made up of predicted reward 0.5 plus exploration bonus 0.353553. `tests/test_controller_linucb.py` recomputes both by code next to this chapter's hand derivation, so the lesson's numbers and the implementation agree by test rather than by transcription.
...
The comparison artifacts put the same policy against the lesson 10 baselines. On the steady world the bandit pays a small exploration tax: total 81.43584 against the optimal static baseline's 83.391417, regret 1.955577. On the drifting world the ranking inverts: the static baseline rides the dead family down to 47.391417, while the bandit adapts to 83.538598: a regret of -23.822717 against the best *fixed* family, negative because an adapting policy can beat any single family chosen with hindsight. And the legacy baseline lands exactly on the hindsight line at 59.715881 by pure luck: its cost-aware rank favors the family that happens to win after the drift. A lucky static rank is indistinguishable from a good one on a single world, which is why comparisons run on more than one.
